...
# from views.__init__ import app, Mongo, page_size, file_path, csrf
import urllib3
import copy
...
def DownloadXls():
    ...
    if tid:  # 有任务id
        ...
        if taskdate == '':
            ...
            response.headers["Content-Disposition"] = "attachment; filename=" + parse.quote(
...
def AddPlugin():
    ...
    if isupload == 'true' and result == 'success':
        ...
        rsp = urllib.request.urlopen(req,parse.urlencode(params))
...
def PullUpdate():
    rsp = 'err'
    f = urllib.request.urlopen('https://sec.ly.com/xunfeng/getlist')
    j = f.read().strip()
    if j:
        try:
...
def installplugin():
    ...
    if item['location'].find('/') == -1:
        urllib.request.urlretrieve('https://sec.ly.com/xunfeng/getplugin?name=' + item['location'], file_path + file_name)
    else:
        urllib.request.urlretrieve(item['location'], file_path + file_name)  # 兼容旧的插件源
    if os.path.exists(file_path + file_name):
        try:

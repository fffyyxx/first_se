...
class vulscan():
    ...
    def set_request(self):
        ...
        if self.plugin_info['plugin']['method'] == 'GET':
            ...
            request1 = urllib.request.Request(url, self.plugin_info['plugin']['data'])
        self.poc_request = request1
        ...
        for task_netloc in task_target:
            while True:
                if int(_thread._count()) < THREAD_COUNT:
                    ...
                    _thread.start_new_thread(vulscan, (task_id, task_netloc, task_plugin))
                    break
                else:
                    time.sleep(2)
